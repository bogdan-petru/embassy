...
use super::controller_registers::{
    ...
    StartAction, StartDrainStep, StopAction, StopStep, TransferFault,
};
...
use session::{Abort, PendingRecovery, Session, SessionRxStep, StartReservation};
...
fn remediate_inner(
    ...
    late_halt_phase = session::retain_recovery_rx_progress(late_halt_phase, regs);
    ...
    if !resolved && (busy || regs.master_busy()) {
        ...
        loop {
            ...
            if let Some(halt) = regs.observe_recovery_halt() {
                ...
                late_halt_phase = session::retain_recovery_discarded_rx_progress(late_halt_phase, regs);
            }
